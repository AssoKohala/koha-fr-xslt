...
<xsl:template name="tag_note327"><!-- contenu 327 -->
      <span class="result_detail">
        <span class="label">Contenu : </span><span class="valeur">
...
<xsl:template name="tag_note330"><!-- résumé 330 -->
      <span class="result_detail">
...
<xsl:template name="tag_note359"><!-- sommaire 359 -->
      <span class="result_detail">
...
<xsl:template name="tag_310"><!-- note de disponibilité (thèse confidentialité ou pas ) et reliure -->
      <span class="result_detail">
        <span class="label">Notes : </span><span class="valeur">
...
<xsl:template name="tag_6xx"><!-- sujets triée alpha -->
    <xsl:if test="marc:datafield[@tag=600 or @tag=601 or @tag=602 or @tag=604 or @tag=605 or @tag=606 or @tag=607 or @tag=608 or @tag=610 or @tag=616]">
      <span class="result_detail">
        ...
        <a><xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=subject%3A(<xsl:value-of select="translate($sujet,',;()','')" />)
           </xsl:attribute>
...
<xsl:template name="tag_7xx"><!-- auteurs -->
    ...
    <xsl:variable name="IdRef" select="marc:sysprefs/marc:syspref[@name='IdRef']"/>
      ...
      <xsl:for-each select="marc:datafield[@tag=700]">
            <a>
               <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=author%3A"<xsl:value-of select="marc:subfield[@code='a']"/><xsl:value-of select="marc:subfield[@code='g']"/><xsl:if test="marc:subfield[@code='b']"><xsl:text>+</xsl:text><xsl:value-of select="marc:subfield[@code='b']"/></xsl:if><xsl:text>"</xsl:text></xsl:attribute>
                <xsl:if test="marc:subfield[@code='3']">
               <xsl:attribute name="ppn"><xsl:value-of select="marc:subfield[@code='3']"/></xsl:attribute>
                </xsl:if>
              <xsl:if test="marc:subfield[@code='a']">
...
<xsl:if test="$IdRef = '1' and marc:subfield[@code=3]"> <!-- le test ne fonctionne pas, et sans test l'url de ne fonctionne pas -->
 <xsl:text> </xsl:text><a>
<xsl:attribute name="href"><xsl:text>/cgi-bin/koha/opac-idref.pl?unimarc3=</xsl:text><xsl:value-of select="marc:subfield[@code=3]"/></xsl:attribute>
...
<!--    <xsl:if test="marc:datafield[@tag=930]">  -->
    <span class="result_detail">
     <span class="label">Collection(s) <a href="#" class="infobulle_c" rel="shadowbox;">(?)<span>Exemples d'écriture d'une collection<br /><br />1980- <br />&#160;&#160;pour un abonnement commencé en 1980 et toujours en cours<br /><br />19, 1965-<br />&#160;&#160;pour un abonnement commencé au numéro 19 de 1965 et toujours en cours<br /><br />1980-2010<br />&#160;&#160;pour un abonnement commencé en 1980 mais arrété en 2010<br /><br />1980-1990 ; 2010-<br />&#160;&#160;pour un abonnement commencé en 1980 et arrété en 1990 ; Puis repris en 2010 et toujours en cours<br /><br />1970-(lac)<br />&#160;&#160;pour un abonnement commencé en 1970 et toujours en cours mais comportant des lacunes</span></a> : </span><span class="valeur">
    ...
    <xsl:variable name="bib" select="marc:subfield[@code='l']" />
    <xsl:variable name="loc" select="marc:subfield[@code='m']" />
    <xsl:variable name="cote" select="marc:subfield[@code='d']" />
    <xsl:if test="$bib = 'BU Tréfilerie'">
      <span>
        <xsl:attribute name="id">demandeMagRevue_<xsl:value-of select="translate($cote,' ','')"/></xsl:attribute>
        <xsl:attribute name="cote"><xsl:value-of select="marc:subfield[@code='d']"/></xsl:attribute>
        <xsl:attribute name="coll"><xsl:value-of select="marc:subfield[@code='h']"/></xsl:attribute>
        <xsl:attribute name="collLA"><xsl:value-of select="marc:subfield[@code='i']"/></xsl:attribute>
        <xsl:attribute name="titre"><xsl:value-of select="//marc:datafield[@tag=200]/marc:subfield[@code='a']"/></xsl:attribute>
        <xsl:attribute name="biblionumber"><xsl:value-of select="//marc:datafield[@tag=999]/marc:subfield[@code='a']"/></xsl:attribute>
        <xsl:attribute name="class">demandeMagRevue</xsl:attribute>
      &#xA0;</span>
    </xsl:if>
   </xsl:for-each>
    </span></span>
...
<xsl:if test="marc:datafield[@tag=992]">
 ...
 <xsl:for-each select="marc:datafield[@tag=992]">
  ...
  <xsl:choose>
   <xsl:when test="marc:subfield[@code=9]">
    <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=an%3A<xsl:value-of select="marc:subfield[@code=9]"/></xsl:attribute>
   </xsl:when>
   <xsl:otherwise>
    <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=subject%3A<xsl:value-of select="marc:subfield[@code='a']"/></xsl:attribute>
   </xsl:otherwise>
  </xsl:choose>
