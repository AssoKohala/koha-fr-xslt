...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:items="http://www.koha.org/items" xmlns:marc="http://www.loc.gov/MARC21/slim" version="1.0" exclude-result-prefixes="marc items">
  <xsl:import href="UNIMARC_Detail_utils_brise_es_pro.xsl" />
      ...
      <div id="catalogue_detail_biblio">
        ...
        <div class="notice">
          ...
          <xsl:if test="marc:datafield[@tag=336]">
            <xsl:call-template name="tag_336" />
            <!-- note -->
          </xsl:if>
          <xsl:if test="marc:datafield[@tag=337]">
            <xsl:call-template name="tag_337" />
            <!-- note -->
          </xsl:if>
          <xsl:if test="marc:datafield[@tag=359]">
            <xsl:call-template name="tag_359" />
            <!-- tdm -->
          </xsl:if>
          ...
          <xsl:if test="marc:datafield[@tag=992]">
            <xsl:call-template name="tag_992" />
            <!-- sujet en3s -->
          </xsl:if>
          <xsl:if test="marc:datafield[@tag=993]">
            <xsl:call-template name="tag_993" />
            <!-- sujet ensase -->
          </xsl:if>
          <xsl:if test="marc:datafield[@tag=998]">
            <xsl:call-template name="tag_998" />
            <!-- bib -->
          </xsl:if>
        </div>
      </div>
    </div>
    <!-- <div style="height:150px;margin-top:50px;"><hr /></div> -->
  </xsl:template>
</xsl:stylesheet>
