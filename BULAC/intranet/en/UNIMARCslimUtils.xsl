...
<!-- Template 45x - Traductions -->
 ...
 <span class="results_summary tag_45x">
...
<!-- Template 46x - Notices liées -->
 ...
 <xsl:param name="label" />
 <xsl:if test="marc:datafield[@tag=$tag]">
 <span class="results_summary tag_46x">
 <span class="label"><xsl:value-of select="$label" />: </span>
 <xsl:for-each select="marc:datafield[@tag=$tag]">
 <span>
 <xsl:call-template name="addClassRtl" />
...
<!-- Problème sur les 46x$9 identifié : rebond vers la mauvaise notice - Recherche désactivée le temps de corriger l'alignement des données
 ...
 <xsl:element name="a"><xsl:attribute name="href">
/cgi-bin/koha/catalogue/search.pl?idx=Local-number,phr&amp;q=<xsl:value-of select="marc:subfield[@code='9']"/></xsl:attribute>
...
<xsl:otherwise>
<xsl:element name="a"><xsl:attribute name="href">
/cgi-bin/koha/catalogue/search.pl?phr&amp;q=<xsl:value-of select="marc:subfield[@code='t']"/>
</xsl:attribute><xsl:value-of select="marc:subfield[@code='t']"/></xsl:element>
</xsl:otherwise>
</xsl:choose>
 <xsl:if test="marc:subfield[@code='e']">:
